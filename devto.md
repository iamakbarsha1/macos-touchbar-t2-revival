---
title: The macOS Touch Bar fix everyone posts cannot work. Here is what actually broke mine.
published: false
description: Every forum says pkill TouchBarServer and killall ControlStrip. I traced a dead Touch Bar on a T2 MacBook Pro from launchd down to the USB bus and found the real cause, plus the three wrong turns I took getting there.
tags: macos, debugging, apple, troubleshooting
---
...
It doesn't work. Not "it didn't work for me", but cannot work in the failure mode most people who search for this are actually in.

**TL;DR:** the Touch Bar is two separate USB devices on the T2 bus. Mine had the backlight (`0x8102`) but not the display (`0x8302`). Everything above that, IOKit, WindowServer, TouchBarServer, ControlStrip, was failing correctly in response. An SMC reset brought the display back. Every fix circulating online operates four layers above the actual break.
...
*Commands, output and the full writeup also live at [github.com/iamakbarsha1/macos-touchbar-t2-revival](https://github.com/iamakbarsha1/macos-touchbar-t2-revival). If you run the verification command on your own T2 Mac and see only the backlight, I'd like to know whether the SMC reset worked for you too.*
